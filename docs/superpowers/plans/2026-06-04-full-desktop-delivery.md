...
- [ ] 写失败 e2e：点赞、取消点赞、收藏、取消收藏。
- [ ] 写失败 e2e：关注和取消关注用户。
- [ ] 写失败 e2e：列出通知、标记已读。
- [ ] 写失败 e2e：创建/读取一对一会话和发送消息。
- [ ] 写失败 e2e：举报主题或评论。
- [ ] 实现对应模块、服务和控制器。
- [ ] 运行新增 API e2e、typecheck、build、lint。
- [ ] 提交：`功能：新增社区互动接口`。
...
- [ ] 写失败 e2e：非管理员访问后台接口返回 403。
- [ ] 写失败 e2e：仪表盘统计返回用户数、主题数、评论数、举报数。
- [ ] 写失败 e2e：处理举报并写入审计日志。
- [ ] 写失败 e2e：用户禁言、封禁、恢复和角色调整。
- [ ] 写失败 e2e：分区创建、编辑、关闭、开放。
- [ ] 写失败 e2e：标签列表和状态管理。
- [ ] 实现 `AdminModule`。
- [ ] 运行 admin e2e、typecheck、build、lint。
- [ ] 提交：`功能：新增管理后台接口`。
...
- [ ] 扩展 seed 和 SQL 演示数据，覆盖新增业务模型。
- [ ] README 更新完整项目说明、初始化数据库、演示账号、API 摘要和验收流程。
- [ ] 验证演示 SQL 可重复导入。
- [ ] 提交：`文档：完善完整交付说明`。

## Task 8: 最终验证和推送

- [ ] 启动 `docker compose up -d postgres redis minio`。
- [ ] 运行 `pnpm db:migrate`、`pnpm db:seed`、导入演示 SQL。
- [ ] 运行 `pnpm test`、`pnpm typecheck`、`pnpm build`、`pnpm lint`、`docker compose config`。
- [ ] 使用浏览器验收前台和后台关键路径。
