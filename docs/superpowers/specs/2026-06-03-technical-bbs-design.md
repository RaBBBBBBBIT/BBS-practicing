...
- CSS variables：前台和后台共享 design tokens，支持浅色和深色模式。
- 主题管理：Next.js 应用使用 `data-theme` 和本地持久化实现 `light`、`dark`、`system` 三种偏好。
...
视觉方向采用现代、简洁、开发者工具感的风格。整体参考 GitHub 官网和 GitHub Discussions 一类开发者产品的中性灰阶、克制蓝色动作色、清晰边框和内容优先，但不复刻 GitHub 的品牌、Logo、图标或具体页面。

完整规范见 `docs/design/ui-guidelines.md`。

核心约定：

- 前台和后台共享同一套 CSS variables design tokens。
- 第一版支持浅色模式、深色模式和跟随系统三种主题偏好。
- 主题切换入口放在前台和后台顶部导航右侧。
- 信息密度控制在中低水平，优先保证阅读、搜索、发帖、评论和审核流程清晰。
- 首页、主题详情和后台仪表盘避免高密度大屏感，不做营销型 hero。
- 圆角控制在 `6px` 到 `8px`，颜色以中性灰阶为主，蓝色作为主要操作色。
- 状态色只用于明确反馈：成功、警告、危险、待处理。
- 代码块、标签、列表、表单和后台表格必须同时适配浅色和深色模式。
...
- 采用低信息密度的 GitHub-like 开发者产品风格，是为了让技术内容、代码块和社区讨论更易读，同时保证课程展示时界面现代但不过度花哨。
